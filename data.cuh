...
class Data{
private:
    std::unique_ptr<value_t[]> data; //智能指针,数据的首地址
    size_t num;//?
    size_t curr_num=0; // the number of current data
...
public:
    ...
    //get the address according to index
    ...
    //brute force to find the real nearest neighbor
    template<typename T>
    dist_t real_nn(T& v) const{
        dist_t minn=1e100;//too big?
    ...
    #endif
    void add(idx_t idx,std::vector<std::pair<int,value>>& value){
        curr_num=std::max(curr_num,idx);
        auto p=get(idx);
    ...
    void dump(std::string file="bfsg.data"){
        ...
        //将 count 个对象写入给定的输出流，每个对象的大小为 size 个字节
        fclose(fp);
    }
    //read
...
//function overload
template<>
dist_t Data::l2_distance(idx_t idx,idx_t& b) const{
    auto pa=get(a);
    auto pb=get(b);
    dist_t ret=0;
    for(int i=0;i<dim;i++){
...
template<>
dist_t Data::negative_consine_distance(idx_t a,idx_t& b) const{
    auto pa=get(a);
    auto pb=get(b);
    dist_t ret=0;
    value_t lena=0,lenb=0;
    for(int i=0;i<dim;i++){
        ret+=(*(pa+i))*(*(pb+i));
        lena+=(*(pa+i))*(*(pa+i));
        lenb+=(*(pb+i))*(*(pb+i));
